...
module('--timeout flag tests for browser mode', { concurrency: false }, (_hooks, moduleMetadata) => {
  test('--timeout=5000 with passing tests completes successfully', async (assert, testMetadata) => {
    const { stdout } = await shell('node cli.js tmp/test/passing-tests.js --timeout=5000', { ...moduleMetadata, ...testMetadata });
  ...
  test('--timeout=1000 still passes for fast tests', async (assert, testMetadata) => {
    const { stdout } = await shell('node cli.js tmp/test/passing-tests.js --timeout=1000', { ...moduleMetadata, ...testMetadata });
