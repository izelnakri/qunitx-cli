...
module('--output flag tests for browser mode', { concurrency: false }, (_hooks, moduleMetadata) => {
  test('--output changes the output directory for built files', async (assert, testMetadata) => {
    ...
    const { stdout } = await shell(`node cli.js tmp/test/passing-tests.js --output=${customOutput}`, { ...moduleMetadata, ...testMetadata });

    assertPassingTestCase(assert, stdout, { testNo: 1, moduleName: '{{moduleName}}' });
    assertTAPResult(assert, stdout, { testCount: 3 });

    const indexExists = await fs.stat(`${customOutput}/index.html`).then(() => true).catch(() => false);
    const testsJsExists = await fs.stat(`${customOutput}/tests.js`).then(() => true).catch(() => false);
