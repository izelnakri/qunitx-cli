...
module('Commands | run | Timings.read', { concurrency: true }, () => {
  test('returns {} when the file does not exist', async (assert) => {
    const result = await Timings.read(`${CWD}/tmp/nonexistent-${randomUUID()}`);
  ...
  test('parses a valid timing cache', async (assert) => {
    ...
    assert.deepEqual(await Timings.read(dir), data);
  });
  ...
  test('returns {} for invalid JSON', async (assert) => {
    ...
    assert.deepEqual(await Timings.read(dir), {});
  });
...
module('Commands | run | Timings.compute', { concurrency: true }, () => {
  test('distributes wall time proportionally by weight', (assert) => {
    ...
    const result = Timings.compute(groups, weights, wallTimes);
