...
import { testSession } from '../../lib/api/index.ts';
...
module('API | testSession | events', { concurrency: true }, () => {
  test('yields runStart, one test per test, then runEnd carrying the result', async (assert) => {
    await withTestSession({ inputs: [PASSING] }, async (session) => {
  ...
  test('a failing suite is a result, not a rejection', async (assert) => {
    await withTestSession({ inputs: [FAILING] }, async (session) => {
      const result = await session.result();
...
module('API | testSession | lifecycle', { concurrency: true }, () => {
  test('nothing runs until the session is consumed', async (assert) => {
    await using output = outputDir('api-session-lazy');
    const permit = await acquireBrowser();
    try {
      const session = await testSession({ inputs: [PASSING], output: output.path });

      // The proof that the run has not started: closing without ever consuming it produces the
      // empty result rather than a suite's worth of tests. If `testSession` ran eagerly this would
      // come back with 3.
      await session.close();
  ...
  test('await using disposes the session at the end of the block', async (assert) => {
    ...
    try {
      {
        await using session = await testSession({ inputs: [PASSING], output: output.path });
...
module('API | testSession | events() as a Stream', { concurrency: true }, () => {
  ...
  test('the session stays a handle — it is not itself a Stream', async (assert) => {
    await withTestSession({ inputs: [PASSING] }, (session) => {
      assert.strictEqual(
