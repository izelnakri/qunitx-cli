...
module('TAP | TAPDisplayFinalResult | output', { concurrency: true }, () => {
  test('emits plan line, all summary lines, and duration', (assert) => {
    const output = captureStdout(() => {
      TAPDisplayFinalResult(
        { testCount: 5, passCount: 3, skipCount: 1, failCount: 1, errorCount: 1 },
        2345,
      );
    });
  ...
  test('output starts and ends with a blank line', (assert) => {
    const output = captureStdout(() => {
      TAPDisplayFinalResult(
        { testCount: 1, passCount: 1, skipCount: 0, failCount: 0, errorCount: 0 },
        100,
      );
    });
  ...
  test('lines appear in the correct order', (assert) => {
    const output = captureStdout(() => {
      TAPDisplayFinalResult(
        { testCount: 3, passCount: 2, skipCount: 0, failCount: 1, errorCount: 1 },
        500,
      );
    });
    ...
    assert.true(idx('# skip 0') < idx('# fail 1'), '# skip before # fail');
  ...
  test('all-passing run shows # fail 0', (assert) => {
    const output = captureStdout(() => {
      TAPDisplayFinalResult(
        { testCount: 10, passCount: 10, skipCount: 0, failCount: 0, errorCount: 0 },
        1000,
      );
    });
  ...
  test('zero-test run emits plan "1..0"', (assert) => {
    const output = captureStdout(() => {
      TAPDisplayFinalResult(
        { testCount: 0, passCount: 0, skipCount: 0, failCount: 0, errorCount: 0 },
        0,
      );
    });
