...
module('Folder Input Tests', { concurrency: false }, (_hooks, moduleMetadata) => {
  ...
  test('works for a single folder input in browser mode with all passing tests', async (assert, testMetadata) => {
    ...
    assertPassingTestCase(assert, stdout, {
      debug: false,
  ...
  test('works for a multiple folders input in browser mode with few failing tests', async (assert, testMetadata) => {
    ...
    try {
      ...
      assertPassingTestCase(assert, cmd.stdout, {
        debug: false,
        moduleName: `${thirdFolderName} | first-module-pass`,
      });
      assertPassingTestCase(assert, cmd.stdout, {
        debug: false,
        moduleName: `${thirdFolderName} | second-module-pass`,
      });
      assertFailingTestCase(assert, cmd.stdout, {
        debug: false,
        moduleName: `${thirdFolderName} | first-module-fail`,
      });
      assertFailingTestCase(assert, cmd.stdout, {
        debug: false,
        moduleName: `${thirdFolderName} | second-module-fail`,
      });
      assertFailingTestCase(assert, cmd.stdout, {
        debug: false,
        moduleName: `${thirdFolderName} | third-module-fail`,
      });
      assertTAPResult(assert, cmd.stdout, { testCount: 24, failCount: 9 });
    }
  });
  ...
  test('works for a single folder input in browser mode with debug and all passing tests', async (assert, testMetadata) => {
    ...
    const { stdout } = await shell(`node cli.js tmp/${folderName} --debug`, {
      ...moduleMetadata,
      ...testMetadata,
    });
  ...
  test('works for a multiple folders input in browser mode with debug and few failing tests', async (assert, testMetadata) => {
    ...
    try {
      await shell(`node cli.js tmp/${firstFolderName} tmp/${secondFolderName} --debug`, {
        ...moduleMetadata,
        ...testMetadata,
      });
    } catch (cmd) {
