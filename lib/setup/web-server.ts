...
function escapeAndInjectTestsToHTML(
  ...
  // Escape </script> inside the runtime code so the HTML parser doesn't close the tag early.
  ...
  // NOTE: remove the escape workaround when simple-html-tokenizer PR gets merged
  return replaceHTMLContentMarker(
    html,
    `${testRuntimeCode.replace(/<\/script>/g, '<\\/script>')}\n<script src="${testBundleUrl}" async></script>`,
  );
}
