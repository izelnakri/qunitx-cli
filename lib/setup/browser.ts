...
export async function setupBrowser(
  ...
  const [server, browser, page] = await (async () => {
    ...
    //   * --coverage arms V8 coverage via page.coverage.startJSCoverage() on this page. That
    //     call fails on the blank page (it belongs to a context Playwright did not create) and
    //     its rejection is swallowed, so the run completes with "no coverable sources found"
    //     and exit 0 rather than an error. Opening our own page keeps coverage working; it is
    //     opt-in and rare, so the fast path still covers the common run.
    const canClaimBlankPage =
      config.state.groupCount === 1 && !config.state.daemon && !config.coverage;
