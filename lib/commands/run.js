...
export default async function run(config) {
  ...
  if (config.watch) {
    ...
    // does not wait for pending writes. stream.end() emits 'finish' only after ALL queued
    // writes have been flushed to the underlying system — stronger than write(callback),
    // which fires when just that one chunk is written.
    // process.exitCode is set first so natural event-loop drain (no zombie handles) also
    // exits with the correct code if the end() callback is somehow bypassed.
