...
export default async function run(config) {
  ...
  if (config.watch) {
    ...
    // Set exitCode immediately so natural event-loop drain uses the correct code
    // even if a subsequent await (browser.close) exits unexpectedly.
    process.exitCode = exitCode;

    // Race browser.close() with an unref'd 10-second timeout to prevent hanging
    // when Firefox/WebKit doesn't respond to the Playwright shutdown command.
    // connectOverCDP browsers disconnect cleanly; launch() browsers may not.
    await Promise.race([
      browser.close().catch(() => {}),
      new Promise((resolve) => {
        const t = setTimeout(resolve, 10000);
    ...
    // Flush stdout before exit: console.log writes are async on pipes and process.exit()
    // does not wait for pending writes. write(callback) fires after this chunk is flushed.
    // The unref'd timer is a last-resort fallback: if the write callback never fires
    // (e.g. EPIPE or a zombie handle from a timed-out browser keeps the loop alive),
    // the timer fires after 5 s and forces exit with the already-saved process.exitCode.
