    ...
    if (runHasFilter) {
      ...
      await Promise.all(
        cachedContent.htmlPathsToRunTests.map((htmlPath) => {
          return runTestInsideHTMLFile(htmlPath, connections, config);
        }),
      );
    }
...
async function runTestInsideHTMLFile(filePath, { page, server, browser }, config) {
  ...
  try {
    ...
    // Race: WebSocket 'done' event (all testEnd messages already flushed by TCP
    // ordering) vs timer fallback for hung/crashed test runs.
    const testsDone = new Promise((resolve) => {
      config._testRunDone = resolve;
    });

    // Set IS_PUPPETEER before any page script runs so QUnit's testEnd/done events
    // are always sent via WebSocket, even if tests start before page.goto resolves.
  ...
  if (!QUNIT_RESULT || QUNIT_RESULT.totalTests === 0) {
    ...
    await failOnNonWatchMode(config.watch, { server, browser });
...
async function failOnNonWatchMode(watchMode = false, connections = {}) {
  if (!watchMode) {
    await Promise.all([
      connections.server && connections.server.close(),
      connections.browser && connections.browser.close(),
