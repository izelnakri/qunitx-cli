...
// Startup safety-net: Chrome WS 'open' must arrive within 9× config.timeout.
// Bumped 3× → 6× → 9× over two iterations:
//   * 3× (60 s) failed JSX on macOS-deno (CI 26042614416)
//   * 6× (120 s) failed JSX again at 121.5 s on macOS-deno (CI 26046813154)
// The 3-way concurrent Chrome launch (one shared Browser, three BrowserContexts
// for the three .tsx/.jsx fixtures) saturates the macos-latest 3-core runner's
// renderer thread; tail latency for WS 'open' has been observed up to ~120 s.
// 9× = 180 s adds a final 60 s of headroom — well clear of the worst observed
// tail. Green-path runs (Chrome opens WS in < 1 s) are unaffected; a genuine
// startup hang now takes 180 s vs the prior 120 s to surface, easily absorbed
// by the per-job CI budgets (15-25 min).
const STARTUP_TIMEOUT_FACTOR = 9;
  ...
  try {
    ...
    if (build.activeRebuild) {
      await Task(build.activeRebuild).ignore(
        'in-flight rebuild — its failure is reported by the caller',
      );
...
export async function flushConsoleHandlers(
  ...
  if (page) await Task(page.evaluate(() => 0)).ignore('CDP queue drain on a possibly-closed page');
...
async function runTestInsideHTMLFile(
  ...
  try {
    ...
    const targetUrl = `http://localhost:${config.port}${filePath}${buildQUnitFilterQuery(config)}`;
