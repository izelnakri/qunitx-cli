 ...
 *
 * Lazy, because it is a {@link Task}: no browser starts until you await it. `await test(…)`
 * resolves the result or throws a {@link RunFailure}; `test(…).result()` hands back the bare
 * union instead, for code that would rather branch than catch.
 *
 * Named `test` because its sibling {@link run} executes ONE file as a plain script. This is the
 * verb that runs a suite; it was called `run` until the script verb needed that name.
 *
 * ```ts
