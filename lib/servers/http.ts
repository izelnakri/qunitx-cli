...
/** `http.ServerResponse` extended with convenience `send` and `json` helpers. */
export interface ExtendedResponse extends http.ServerResponse {
  /** Sends a plain-text response. */
  send: (data: string) => void;
  /** Sends a JSON-serialized response. */
  json: (data: unknown) => void;
}

/** `http.IncomingMessage` extended with parsed path, query, and route params. */
export interface ExtendedRequest extends http.IncomingMessage {
  /** Parsed URL pathname (no query string). */
  path: string;
  /** Parsed query-string key/value pairs. */
  query: Record<string, string>;
  /** Route parameter key/value pairs extracted from the matched route pattern. */
  params: Record<string, string>;
}

/** Route handler function signature for registered GET/POST/etc. routes. */
export type RouteHandler = (req: ExtendedRequest, res: ExtendedResponse) => void | Promise<void>;
/** Middleware function signature — call `next()` to continue the chain. */
export type Middleware = (req: ExtendedRequest, res: ExtendedResponse, next: () => void) => void;
...
export default class HTTPServer {
  ...
  static serve(
    ...
    return new Promise((resolve, reject) => {
      ...
      (server as NodeServerWithWSS).wss.on('error', (error) => {
  ...
  constructor() {
    ...
    this._server = http.createServer((req, res) => {
      ...
      extRes.send = (data: string) => {
        res.setHeader('Content-Type', 'text/plain');
        res.end(data);
      };
      extRes.json = (data: unknown) => {
        res.setHeader('Content-Type', 'application/json');
        res.end(JSON.stringify(data));
      };

      return this.#handleRequest(extReq, extRes);
    });
  ...
  #handleRequest(req: ExtendedRequest, res: ExtendedResponse): void {
  ...
  #runMiddleware(req: ExtendedRequest, res: ExtendedResponse, callback: RouteHandler): void {
    let index = 0;
    const next = () => {
